...
#IF YOU ARE USING THE OUTPUT FROM BASH SCRIPT DELETE FIRST LINE OF OUTPUT BEFORE RUNNING THIS> 
#
#first line should be the one that starts with "geneid"
#also sort geneid and make sure there isn't any weird excel things happening when opening (i.e.  non-unique value...'row.names': ‘1-Mar’)

# ==============================================================================
# Configuration (EDIT THESE FOR EACH PROJECT, or override via params)
# ==============================================================================
output_dir     <- "deseq2_outputs"
counts_file    <- "GRCg7b_GOSrealignment_9_25_featureCounts_gene_name_with_ensembl.txt"   # CSV/TSV with 'Geneid' + sample columns
metadata_file  <- "GRCg7b_metadata.csv"                               # CSV with rownames = sample IDs; must include 'condition'
...
if (run_gsea) {
  ...
  if (ok) {
    OrgDb <- tryCatch(getExportedValue(OrgDb_pkg, OrgDb_pkg),
...
to_entrez <- function(vec_ids, keyType_in, OrgDb) {
  ...
  suppressWarnings({
    df <- tryCatch(
      bitr(vec_ids, fromType = keyType_in, toType = "ENTREZID", OrgDb = OrgDb),
      error = function(e) NULL
    )
  })
...
run_contrast <- function(dds_obj, group1, group0, do_gsea = TRUE, shrink_method = "apeglm") {
  ...
  # ---- Extra exports with alternative row labels ----
  gann_sub <- gene_annot[rownames(out), c("SYMBOL","Ensembl_GeneID"), drop = FALSE]

  # Ensembl-labeled
  ens_lab <- gann_sub$Ensembl_GeneID
  ens_lab[is.na(ens_lab) | !nzchar(ens_lab)] <- rownames(out)   # fallback to Geneid
  ...
  # SYMBOL-labeled (NOT 'gene-xxxx')
  sym_lab <- gann_sub$SYMBOL
  sym_lab[is.na(sym_lab) | !nzchar(sym_lab)] <- rownames(out)   # fallback to Geneid
  ...
  if (do_gsea && !is.null(OrgDb)) {
    ...
    if (!is.null(sel)) {
      ...
      # GO GSEA
      gse_go <- tryCatch(
        gseGO(geneList = ranked_named, keyType = keyType, ont = "ALL",
      ...
      map_df <- tryCatch(
        bitr(names(ranked_named), fromType = keyType, toType = "ENTREZID", OrgDb = OrgDb),
        error = function(e) NULL
      )
