...
format_pathway_output_label <- function(node_id, default_label = node_id) {
  node_id <- as.character(node_id)
  default_label <- as.character(default_label)
  out <- default_label
  out[!is.na(node_id) & toupper(node_id) == "RIPK3"] <- "GENEID 415708 (RIP2K-like)"
  out
}
...
get_pathway_config <- function(pathway) {
  ...
  if (pathway == "combined_death") {
    ...
    death_nodes <- tribble(
      ...
      "TRADD", "TRADD", "TRADD", 9.2, -0.2, "Death modules", "Cytosol",
      "FADD", "FADD", "FADD", 10.2, 0.4, "Apoptosis", "Cytosol",
      "CASP8", "CASP8", "CASP8", 11.2, 0.4, "Apoptosis", "Cytosol",
      "CASP7", "CASP7", "CASP7", 12.2, 0.9, "Apoptosis", "Cytosol",
      "CASP3", "CASP3", "CASP3", 13.2, 0.4, "Apoptosis", "Cytosol",
      "BID", "BID", "BID", 11.2, -0.4, "Apoptosis", "Mitochondria",
      "BAX", "BAX", "BAX", 12.2, -0.8, "Apoptosis", "Mitochondria",
      "CYCS", "CYCS", "CYCS", 13.2, -0.8, "Apoptosis", "Mitochondria",
      "APAF1", "APAF1", "APAF1", 14.2, -0.8, "Apoptosis", "Cytosol",
      "CASP9", "CASP9", "CASP9", 15.2, -0.8, "Apoptosis", "Cytosol",
      "RIPK1", "RIPK1", "RIPK1", 10.2, -1.8, "Necroptosis", "Cytosol",
      "RIPK3", "RIPK3", "RIPK3", 11.2, -1.8, "Necroptosis", "Cytosol",
      "MLKL", "MLKL", "MLKL", 12.2, -1.8, "Necroptosis", "Plasma membrane"
    )
    ...
    compartment_boxes <- tibble::tribble(
      ...
      "Cytosol",                 9.05,  12.45,  -2.10,   1.35,  list(c("TRADD", "FADD", "CASP8", "CASP7", "RIPK1", "RIPK3")),                 0,              0,              0.30,
      "Nucleus",                 2.15,   6.95,   1.05,   3.85,  list(c("IRF7", "IRF3", "STAT1", "STAT2", "IRF9", "NFKB1", "RELA")),           0,              0,              0.30,
      "Mitochondria",           12.05,  13.85,  -1.30,  -0.30,  list(c("BAX", "CYCS")),                                                        0,              0,              0.30,
      "ER/Golgi",                0.15,   1.85,   1.05,   2.55,  list(c("STING1")),                                                             0,              0.30,           0.30,
      "Plasma membrane (MLKL)", 11.65,  12.75,  -2.10,  -1.40,  list(c("MLKL")),                                                               0,              0,              0.25
    )
...
build_pathway_node_table <- function(config, de_by_symbol, p_cutoff = 0.05) {
  ...
  nodes %>%
    ...
    mutate(
      ...
      gene_id = ensembl_id,
      label = format_pathway_output_label(node_id, label),
      output_symbol = format_pathway_output_label(node_id, symbol_key)
    ) %>%
    distinct(node_id, .keep_all = TRUE)
}
...
write_pathway_outputs <- function(prefix, outdir, plot_obj, node_tbl, de_annot) {
  ...
  node_tbl %>%
    filter(node_status == "Missing gene") %>%
    dplyr::transmute(symbol = output_symbol, label = label, module = module) %>%
    readr::write_tsv(missing_file, na = "NA")
...
make_interpretation_panel <- function(nodes_annot,
  ...
  validation_text <- paste(
    ...
    paste0(
      "- Necroptosis: pMLKL, ",
      format_pathway_output_label("RIPK3"),
      "/MLKL localization, MLKL membrane translocation."
    ),
