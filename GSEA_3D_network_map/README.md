...
| `--gene-sets` value | Library |
|---------------------|---------|
| `H` | MSigDB Hallmark (default — 50 well-curated sets) |
| `KEGG` | KEGG 2019 Human |
| `REACTOME` | Reactome 2022 |
| `C5_GO_BP` | GO Biological Process |
| `C5_GO_MF` | GO Molecular Function |
| `C5_GO_CC` | GO Cellular Component |
| `WIKIPATHWAYS` | WikiPathways Human |
| `/path/to/file.gmt` | Any local GMT file |

Gene sets are downloaded from Enrichr automatically. MSigDB gene sets use human gene symbols — these match well for chicken data because most chicken orthologs share the same HGNC symbol.
