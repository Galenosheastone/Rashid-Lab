...
fig2.suptitle("Pathway Scores by Group — All Three Methods\n"
              "Stars = nominal (unadjusted) Kruskal-Wallis p < 0.05  |  No comparisons survive BH correction at n=4/group",
...
# Stars legend
axes3[2].text(1.05, 0.98, "Stars = nominal (unadjusted) p\n* < .05  ** < .01  *** < .001  · < .10\n⚠ No pairs survive BH correction\n  at n=4/group",
...
fig3.suptitle("Pathway Score Range by Group — Median per Group\n"
              "Stars = most significant pairwise comparison (nominal unadjusted p — no pairs survive BH correction at n=4/group)",
...
fig4.suptitle("GSVA Pathway Scores by Group\n"
              "Boxes = IQR; points = individual samples; brackets = nominal (unadjusted) p — no pairs survive BH correction at n=4/group",
...
ax_sig.set_xlabel("–log₁₀(Kruskal-Wallis nominal p, unadjusted)", fontsize=12, labelpad=8)
ax_sig.set_title("Omnibus Significance per Method\n(nominal p; dashed = BH-uncorrected thresholds)", fontsize=12,
...
fig5.suptitle("Method Comparison — Median Scores & Omnibus Significance (nominal p, unadjusted)",
