...
# ══════════════════════════════════════════════════════════════════════════════
# FIGURE 1 — Gene set coverage & membership overview
# ══════════════════════════════════════════════════════════════════════════════
...
ax.set_xlim(0, 115)
ax.set_xlabel("Gene set coverage (%)", fontsize=11)
ax.set_title("Gene Set Coverage", fontsize=13, fontweight="bold", pad=10)
...
# Also grab unmatched per pathway
pw_unmatched = {}
for _, row in cov.iterrows():
    if pd.notna(row.unmatched_genes) and row.unmatched_genes:
        ug = [g.strip() for g in str(row.unmatched_genes).split(";") if g.strip()]
    else:
        ug = []
    pw_unmatched[row.set_name] = set(ug)

# Sort genes by how many pathways they appear in
gene_counts = {g: sum(1 for pw in pw_genes.values() if g in pw) for g in all_genes}
genes_ordered = sorted(all_genes, key=lambda g: (-gene_counts[g], g))

pathways_order = list(cov.set_name)  # original order

# Build matrix: genes × pathways
mat = np.zeros((len(genes_ordered), len(pathways_order)))
for j, pw in enumerate(pathways_order):
    for i, g in enumerate(genes_ordered):
        mat[i, j] = 1 if g in pw_genes[pw] else 0

# Count membership per gene
membership = np.sum(mat, axis=1)

# Color: single membership = dark; multi = highlighted
...
for i, g in enumerate(genes_ordered):
    for j, pw in enumerate(pathways_order):
        if mat[i, j] == 1:
            ...
            ax2.scatter(j, i, s=55, color=c, zorder=3, linewidths=0,
                        alpha=0.85)

# Grid
for j in range(len(pathways_order)):
    ax2.axvline(j, color="#E2E8F0", lw=0.5, zorder=1)
for i in range(len(genes_ordered)):
    ax2.axhline(i, color="#F1F5F9", lw=0.3, zorder=1)

ax2.set_xticks(range(len(pathways_order)))
ax2.set_xticklabels([pwy_label(p) for p in pathways_order],
                    rotation=40, ha="right", fontsize=8.5)
ax2.set_yticks(range(len(genes_ordered)))
ax2.set_yticklabels(genes_ordered, fontsize=7.5)
ax2.set_xlim(-0.5, len(pathways_order) - 0.5)
ax2.set_ylim(-0.5, len(genes_ordered) - 0.5)
ax2.set_title("Gene Membership Across Pathways", fontsize=13,
               fontweight="bold", pad=10)
ax2.spines[["top","right","left","bottom"]].set_visible(False)
ax2.tick_params(length=0)

legend2 = [
    mpatches.Patch(color="#94A3B8", label="In 1 pathway"),
    mpatches.Patch(color="#F59E0B", label="In 2 pathways"),
    mpatches.Patch(color="#EF4444", label="In 3 pathways"),
    mpatches.Patch(color="#7C3AED", label="In 4+ pathways"),
]
ax2.legend(handles=legend2, loc="upper right", fontsize=7.5,
           framealpha=0.95, edgecolor="#CBD5E1", title="Shared genes",
           title_fontsize=8)

fig1.suptitle("Gene Set Composition & Coverage", fontsize=15, fontweight="bold",
              y=1.01, x=0.5, color="#1E293B")
fig1.tight_layout(rect=[0, 0, 1, 0.98])
fig1.savefig(OUT / "fig1_coverage_and_membership.pdf", bbox_inches="tight",
             facecolor=fig1.get_facecolor())
fig1.savefig(OUT / "fig1_coverage_and_membership.png", dpi=180,
...
# ══════════════════════════════════════════════════════════════════════════════
# FIGURE 5 — Multi-method consensus: scatter + agreement
# ══════════════════════════════════════════════════════════════════════════════
# Build per-pathway, per-group median score for each method
...
gs5 = gridspec.GridSpec(1, 4, figure=fig5, wspace=0.35)

pairs = [("GSVA","ssGSEA"), ("GSVA","singscore"), ("ssGSEA","singscore")]

for pi, (m1, m2) in enumerate(pairs):
    ax = fig5.add_subplot(gs5[pi])
    ax.set_facecolor("#F8FAFC")
    for grp in GROUPS:
        sub = mm[mm.group == grp]
        ax.scatter(sub[m1], sub[m2], color=GRP_COL[grp], s=80, zorder=4,
                   edgecolors="white", linewidths=0.7, alpha=0.9, label=grp)
        # label pathway name
        for _, row in sub.iterrows():
            ax.text(row[m1], row[m2], "  " + pwy_label(row.pathway),
                    fontsize=6.5, color="#475569", va="center")

    # Correlation line
    from scipy import stats
    r, p = stats.pearsonr(mm[m1], mm[m2])
    xmin, xmax = ax.get_xlim()
    xs = np.linspace(min(mm[m1]), max(mm[m1]), 100)
    slope, intercept, *_ = stats.linregress(mm[m1], mm[m2])
    ax.plot(xs, slope * xs + intercept, color="#94A3B8", lw=1.2,
            ls="--", zorder=2)
    ...
    ax.axhline(0, color="#E2E8F0", lw=0.7, zorder=1)
    ax.axvline(0, color="#E2E8F0", lw=0.7, zorder=1)
    ax.set_xlabel(m1, fontsize=11, fontweight="bold",
                  color=METHOD_COL[m1])
    ax.set_ylabel(m2, fontsize=11, fontweight="bold",
                  color=METHOD_COL[m2])
...
fig5.axes[0].legend(handles=handles5, loc="lower right", fontsize=8.5,
                    framealpha=0.95, edgecolor="#CBD5E1")

# 4th panel: per-pathway p-value comparison across methods
ax4 = fig5.add_subplot(gs5[3])
...
for st, method, color in [(st_g, "GSVA", METHOD_COL["GSVA"]),
    ...
    ax4.plot(xs, ys, "o-", color=color, lw=1.5, ms=7,
             markeredgecolor="white", markeredgewidth=0.5, label=method,
             alpha=0.85)

ax4.axvline(-np.log10(0.05), color="#EF4444", lw=1, ls="--")
ax4.axvline(-np.log10(0.1), color="#F59E0B", lw=0.8, ls=":")
ax4.text(-np.log10(0.05) + 0.04, len(pw_order) - 0.5, "p=.05",
         fontsize=7.5, color="#EF4444", va="top")
ax4.text(-np.log10(0.1) + 0.04, len(pw_order) - 0.5, "p=.10",
         fontsize=7.5, color="#F59E0B", va="top")

ax4.set_yticks(range(len(pw_order)))
ax4.set_yticklabels(pw_labels, fontsize=8.5)
ax4.set_xlabel("–log₁₀(Kruskal-Wallis p)", fontsize=9.5)
ax4.set_title("Omnibus Significance\nper Method", fontsize=10.5,
               fontweight="bold", color="#1E293B", pad=8)
ax4.spines[["top","right"]].set_visible(False)
ax4.legend(fontsize=8, framealpha=0.95, edgecolor="#CBD5E1",
           loc="lower right")
ax4.tick_params(axis="y", length=0)

fig5.suptitle("Method Comparison — Median Scores & Omnibus Significance",
              fontsize=14, fontweight="bold", y=1.03, color="#1E293B")
