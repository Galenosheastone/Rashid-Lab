...
# ══════════════════════════════════════════════════════════════════════════════
# FIGURE 1b — Gene membership matrix (tall standalone)
# ══════════════════════════════════════════════════════════════════════════════
...
# Build matrix: genes (rows) × pathways (cols)
...
# Size: 0.32" per gene row, fixed width for pathways + gene labels
n_genes    = len(genes_ordered)
n_pathways = len(pathways_order)
row_height = 0.32          # inches per gene
fig_height = max(14, n_genes * row_height + 4.0)   # +4 for margins/xlabel
fig_width  = 13.0
...
for i in range(n_genes):
    color = "#F1F5F9" if i % 2 == 0 else "#FFFFFF"
    ax2.axhspan(i - 0.5, i + 0.5, color=color, zorder=1)

# Vertical pathway dividers
for j in range(n_pathways):
    ax2.axvline(j, color="#E2E8F0", lw=0.6, zorder=2)

# Dots — size scales with figure height so they stay proportional
dot_size = max(35, int(row_height * 220))
...
# Pathway column headers (x-axis top + bottom)
ax2.set_xticks(range(n_pathways))
ax2.set_xticklabels([pwy_label(p) for p in pathways_order],
                    rotation=40, ha="right", fontsize=10.5)
ax2.xaxis.set_tick_params(length=0)

# Secondary x-axis on top mirroring pathway names
ax2_top = ax2.twiny()
ax2_top.set_xlim(ax2.get_xlim())
ax2_top.set_xticks(range(n_pathways))
ax2_top.set_xticklabels([pwy_label(p) for p in pathways_order],
                         rotation=40, ha="left", fontsize=10.5)
ax2_top.xaxis.set_tick_params(length=0)

# Gene labels (y-axis) — font sized to row height
gene_fontsize = min(10.5, max(7.5, row_height * 28))
ax2.set_yticks(range(n_genes))
ax2.set_yticklabels(genes_ordered, fontsize=gene_fontsize, fontfamily="monospace")
ax2.tick_params(axis="y", length=0)

ax2.set_xlim(-0.5, n_pathways - 0.5)
ax2.set_ylim(-0.5, n_genes - 0.5)
ax2.spines[:].set_visible(False)

# Section dividers between membership tiers
tier_boundaries = []
...
for yb in tier_boundaries:
    ax2.axhline(yb, color="#475569", lw=0.8, ls="--", zorder=4, alpha=0.5)

# Tier labels on right margin
tier_starts = [0] + [int(b + 0.5) for b in tier_boundaries]
tier_ends   = [int(b - 0.5) for b in tier_boundaries] + [n_genes - 1]
for ts, te in zip(tier_starts, tier_ends):
    ...
    ax2.text(n_pathways - 0.3, mid_y, label,
             fontsize=8.5, color="#64748B", va="center", ha="left",
             style="italic")
...
ax2.legend(handles=legend2, loc="upper right", fontsize=9.5,
           framealpha=0.95, edgecolor="#CBD5E1",
           title="Gene pathway membership", title_fontsize=10,
           bbox_to_anchor=(1.0, 1.0))

ax2.set_title("Gene Membership Across Pathways", fontsize=14,
               fontweight="bold", pad=14, color="#1E293B")

fig1b.tight_layout(rect=[0, 0, 0.92, 1])   # leave right margin for tier labels
