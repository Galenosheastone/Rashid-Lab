...
_GENE_SET_ALIASES = {
    ...
    'REACTOME_GGA':   'REACTOME_GGA',       # Gallus gallus — fetched from Reactome Content Service
    'BIOCARTA':       'BioCarta_2016',
    'WIKIPATHWAYS':   'WikiPathways_2019_Human',
    'WP':             'WikiPathways_2019_Human',
    'WP_GGA':         'WP_GGA',             # Gallus gallus — fetched from WikiPathways GMT
    'WIKIPATHWAYS_GGA': 'WP_GGA',           # alias
}
...
def _fetch_reactome_gga():
    """
    Fetch Reactome pathway gene sets for Gallus gallus via bulk mapping files.

    Strategy — 2 HTTP requests total:
      1. UniProt2Reactome_All_Levels.txt  — all species pathway→protein mappings
         (filter to 'Gallus gallus' lines to get pathway_name → set of UniProt IDs)
      2. UniProt streaming search         — all GGA UniProt accession → gene symbol
         (organism_id:9031, fields=accession,gene_names)

    Reactome GGA pathways are computationally inferred from human via PANTHER
    orthology (stable IDs starting with R-GGA-).  The chicken UniProt entries
    involved therefore tend to carry HGNC-style gene names (e.g. TP53, CASP3)
    which match the DESeq2 CSV gene column directly.
    """
    import urllib.request

    def http_get(url, timeout=120):
        req = urllib.request.Request(url, headers={'User-Agent': 'gsea3d/1.0'})
    ...
    # 1. Download bulk Reactome UniProt→pathway mapping (all species, ~200 MB text)
    print('      Fetching Reactome UniProt→pathway bulk mapping…')
    reactome_url = ('https://reactome.org/download/current/'
                    'UniProt2Reactome_All_Levels.txt')
    try:
        raw = http_get(reactome_url)
    except Exception as e:
        sys.exit(f'ERROR: Could not download Reactome mapping file: {e}')

    # Format: UniProt_ID \t Reactome_ID \t URL \t Pathway_Name \t Evidence \t Species
    pw2uni = defaultdict(set)   # pathway_name → set of UniProt accessions
    for line in raw.splitlines():
        parts = line.split('\t')
        if len(parts) < 6:
            continue
        species = parts[5].strip()
        if 'Gallus gallus' not in species:
            continue
        uniprot_id   = parts[0].strip()
        pathway_name = parts[3].strip()
        if uniprot_id and pathway_name:
            pw2uni[pathway_name].add(uniprot_id)

    print(f'      Found {len(pw2uni)} Gallus gallus Reactome pathways across '
          f'{sum(len(v) for v in pw2uni.values())} protein-pathway links')

    # 2. Fetch accession→gene_name for all Gallus gallus UniProt entries
    print('      Fetching Gallus gallus UniProt gene symbols (streaming)…')
    uniprot_url = ('https://rest.uniprot.org/uniprotkb/stream'
                   '?query=organism_id:9031'
                   '&fields=accession,gene_names'
                   '&format=tsv')
    try:
        tsv_data = http_get(uniprot_url, timeout=180)
    except Exception as e:
        sys.exit(f'ERROR: Could not download UniProt gene symbols: {e}')

    acc2sym = {}   # UniProt accession → HGNC-style gene symbol (first entry)
    for line in tsv_data.splitlines():
        if line.startswith('Entry') or not line.strip():
            continue
        parts = line.split('\t')
        if len(parts) < 2:
            continue
        ...
        if not genes_raw:
            continue
        # gene_names field: space-separated, primary gene name first
        sym = genes_raw.split()[0].strip().rstrip(';').rstrip(',')
        if is_valid_symbol(sym):
            acc2sym[acc] = sym.upper()

    print(f'      Mapped {len(acc2sym)} UniProt accessions to gene symbols')

    # Build final gene set dict — filter to reasonable pathway sizes
    gs_dict = {}
    for pw_name, uni_ids in pw2uni.items():
        syms = {acc2sym[uid] for uid in uni_ids if uid in acc2sym}
        if 10 <= len(syms) <= 500:
            gs_dict[pw_name] = list(syms)

    print(f'      Built {len(gs_dict)} Reactome Gallus gallus gene sets '
          f'(10–500 genes, after symbol conversion)')
    return gs_dict


def _fetch_wikipathways_gga():
    """
    Fetch WikiPathways gene sets for Gallus gallus from their monthly GMT release.

    Strategy — 2 HTTP requests total:
      1. WikiPathways GMT file for Gallus gallus (genes encoded as NCBI Entrez IDs)
      2. NCBI Gallus_gallus.gene_info.gz    — Entrez ID → official gene symbol

    GMT format:
      PathwayName%WPID%date%organism \\t URL \\t EntrezID1 \\t EntrezID2 ...
    """
    import urllib.request
    import gzip
    import io
    ...
    # 1a. Try to find the GMT URL (most-recent 10th-of-month release)
    today = date.today()
    if today.day >= 10:
        release = today.replace(day=10)
    else:
        first = today.replace(day=1)
        prev_month = first - timedelta(days=1)
        release = prev_month.replace(day=10)

    gmt_filename = (f"wikipathways-{release.strftime('%Y%m%d')}"
                    f"-gmt-Gallus_gallus.gmt")
    gmt_url = f"https://data.wikipathways.org/current/gmt/{gmt_filename}"
    print(f'      Fetching WikiPathways GMT: {gmt_filename}…')

    gmt_bytes = None
    try:
        gmt_bytes = http_get_bytes(gmt_url)
    except Exception:
        # Try previous month's release as fallback
        first2 = release.replace(day=1)
        prev2  = (first2 - timedelta(days=1)).replace(day=10)
        gmt_filename2 = (f"wikipathways-{prev2.strftime('%Y%m%d')}"
                         f"-gmt-Gallus_gallus.gmt")
        gmt_url2 = f"https://data.wikipathways.org/current/gmt/{gmt_filename2}"
        print(f'      First attempt failed; trying {gmt_filename2}…')
        try:
            gmt_bytes = http_get_bytes(gmt_url2)
        except Exception as e:
            sys.exit(f'ERROR: Could not download WikiPathways Gallus gallus GMT: {e}')

    # 1b. Parse GMT — collect Entrez IDs per pathway
    pw_entrez = {}   # clean_pathway_name → list of entrez_id strings
    for line in gmt_bytes.decode('utf-8', errors='replace').splitlines():
        if not line.strip():
            continue
        cols = line.split('\t')
        if len(cols) < 3:
        ...
        # Strip WikiPathways suffix: "Name%WPID%date%organism" → "Name"
        clean_name = raw_name.split('%')[0].strip()
        # cols[1] is the URL; cols[2:] are Entrez IDs
        entrez_ids = [c.strip() for c in cols[2:] if c.strip().isdigit()]
        if entrez_ids:
            pw_entrez[clean_name] = entrez_ids

    print(f'      Parsed {len(pw_entrez)} WikiPathways Gallus gallus pathways')

    # 2. Download NCBI gene_info to convert Entrez IDs → gene symbols
    gene_info_url = ('https://ftp.ncbi.nlm.nih.gov/gene/DATA/GENE_INFO/'
                     'Non-mammalian_vertebrates/Gallus_gallus.gene_info.gz')
    print('      Fetching NCBI Gallus_gallus.gene_info.gz…')
    try:
        gz_bytes = http_get_bytes(gene_info_url, timeout=120)
    except Exception as e:
        sys.exit(f'ERROR: Could not download NCBI gene_info: {e}')

    entrez2sym = {}
    with gzip.open(io.BytesIO(gz_bytes), 'rt', encoding='utf-8',
        ...
        for line in fh:
            ...
            if len(parts) < 3:
                continue
            # col 0=tax_id, col 1=GeneID, col 2=Symbol
            gid = parts[1].strip()
            sym = parts[2].strip()
            if is_valid_symbol(sym):
                entrez2sym[gid] = sym.upper()

    print(f'      Mapped {len(entrez2sym)} Entrez IDs to gene symbols')

    # Build final gene set dict
    gs_dict = {}
    for pw_name, entrez_ids in pw_entrez.items():
        syms = list({entrez2sym[eid] for eid in entrez_ids if eid in entrez2sym})
        if len(syms) >= 5:
            gs_dict[pw_name] = syms

    print(f'      Built {len(gs_dict)} WikiPathways Gallus gallus gene sets '
          f'(≥5 genes after Entrez→symbol conversion)')
    return gs_dict
...
def _resolve_gene_sets(gene_sets_arg):
    ...
    # Reactome Gallus gallus — fetched from Reactome bulk mapping + UniProt
    if resolved == 'REACTOME_GGA':
        try:
            gs_dict = _fetch_reactome_gga()
            if gs_dict:
                try:
                    cache_file.write_text(_json.dumps(gs_dict, ensure_ascii=False), encoding='utf-8')
                    print(f"      Cached to {cache_file}")
            ...
            sys.exit(f"ERROR: Could not fetch Reactome Gallus gallus: {e}")

    # WikiPathways Gallus gallus — fetched from WikiPathways GMT + NCBI gene_info
    if resolved == 'WP_GGA':
        try:
            gs_dict = _fetch_wikipathways_gga()
            if gs_dict:
                try:
                    cache_file.write_text(_json.dumps(gs_dict, ensure_ascii=False), encoding='utf-8')
                    print(f"      Cached to {cache_file}")
            ...
            sys.exit(f"ERROR: Could not fetch WikiPathways Gallus gallus: {e}")
...
<div id="layout">
  <div id="control-panel">
    ...
    <div class="panel-section">
      ...
      <div class="slider-row" style="margin-top:8px;">
        <label>Node Spread <span id="spread-slider-val">80</span></label>
        <input type="range" id="spread-slider" min="20" max="300" step="10" value="80">
      </div>
    </div>
...
function buildGraph() {{
  ...
  try {{
    ...
    if (chargeFn) chargeFn.strength(-80);
    const linkFn = Graph.d3Force('link');
    if (linkFn) linkFn.distance(40).strength(0.5);
  ...
  // Remove zoom-in limit so the camera can get very close to nodes
  try {{
    const controls = Graph.controls();
    if (controls) controls.minDistance = 10;
  }} catch(e) {{ console.warn('Controls setup error:', e); }}
...
function onNodeClick(node) {{
  ...
  Graph.cameraPosition(
    {{ x: nx + dist * 0.7, y: ny + dist * 0.3, z: nz + dist }},
    node, 800
  );
  // Update orbit pivot so scroll-zoom and rotation center on the clicked node
  try {{
    const controls = Graph.controls();
    if (controls) {{
...
function buildControls() {{
  ...
  document.getElementById('spread-slider').addEventListener('input', function() {{
    ...
    document.getElementById('spread-slider-val').textContent = val;
    try {{
      const chargeFn = Graph.d3Force('charge');
      if (chargeFn) {{
        chargeFn.strength(-val);
        Graph.d3ReheatSimulation();
      }}
    }} catch(e) {{ console.warn('Spread slider error:', e); }}
  }});
...
_LIBRARY_MENU = [
    ...
    ('REACTOME',     'Reactome 2022            — ~1500 detailed pathway reactions (human)'),
    ('REACTOME_GGA', 'Reactome Gallus gallus   — chicken pathways via Reactome API'),
    ...
    ('WP_GGA',       'WikiPathways Gallus gallus — chicken community-curated pathways'),
]


def _prompt_gene_sets(default):
    """Interactively ask the user to choose a gene set library. Returns the chosen key."""
    _custom_num = len(_LIBRARY_MENU) + 1
    print("\n┌─ Gene Set Library ──────────────────────────────────────────────────┐")
    for i, (key, desc) in enumerate(_LIBRARY_MENU, 1):
        marker = ' ◀ default' if key == default else ''
        print(f"│  {i})  {desc}{marker}")
    print(f"│  {_custom_num})  Custom .gmt file — enter path when prompted")
    print(f"└─────────────────────────────────────────────────────────────────────┘")

    while True:
        try:
            raw = input(f"Choose [1–{_custom_num}, or press Enter for default '{default}']: ").strip()
        ...
        print(f"      Invalid choice — enter a number 1–{_custom_num} or press Enter.")
...
def main():
    ...
    #   'REACTOME_GGA'  — Reactome Gallus gallus (chicken-specific, via Reactome Content Service)
    ...
    #   'WP_GGA'        — WikiPathways Gallus gallus (chicken-specific, downloaded GMT)
    ...
    parser.add_argument('--gene-sets', default=None,
                        help='MSigDB collection name (H, KEGG, KEGG_GGA, REACTOME, REACTOME_GGA, '
                             'C5_GO_BP, C5_GO_MF, C5_GO_CC, WIKIPATHWAYS, WP_GGA) '
